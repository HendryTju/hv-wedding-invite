...
export async function loginUser(email, password) {
  ...
  if (isEmulated) {
    // Special dev trick: typing "host@wedding.com" logs in as host if user doesn't exist
    if (email === "host@wedding.com") {
      const users = getLocalUsers();
      ...
      if (firebaseUser) {
        try {
          ...
          if (userSnap.exists()) {
            ...
            if (data.email === "host@wedding.com" && data.role !== "host") {
              console.log("Auto-elevating host@wedding.com to host role...");
              await updateDoc(doc(db, "users", firebaseUser.uid), { role: "host" });
              data.role = "host";
            }
            callback(data);
          } else {
            // If the host@wedding.com logged in but their user profile document is missing:
            if (firebaseUser.email === "host@wedding.com") {
